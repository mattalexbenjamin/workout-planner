...
export function calculateFatigueImpact(log) {
  const profiles = {
  ...
  const durationFactor = Math.min(4.0, (log.duration || 60) / 60);
  const intensityFactor = (log.intensity || 5) / 10;
  const globalScale = durationFactor * intensityFactor;

  if (log.type === "lifting" || log.type === "workout" || log.category === "weightlifting") {
    let totalVolume = 0;
    ...
    let p = { ...(profiles[log.type || log.category] || { legs: 3.0, back: 2.0, chest: 1.5, shoulders: 2.0, arms: 1.5, core: 2.0, fatigue: 3.0 }) };

    // Dynamic Volleyball Drill Focus Adjustments
    if ((log.category === "volleyball" || log.type === "volleyball") && Array.isArray(log.exercises) && log.exercises.length > 0) {
      let setFocus = 0, hitFocus = 0, blockFocus = 0, digFocus = 0, serveFocus = 0;
      log.exercises.forEach(exObj => {
        const name = (typeof exObj === 'string' ? exObj : exObj.name || "").toLowerCase();
        if (name.includes('set') || name.includes('hand')) setFocus++;
        if (name.includes('spike') || name.includes('attack') || name.includes('hit') || name.includes('cut')) hitFocus++;
        if (name.includes('block') || name.includes('jump')) blockFocus++;
        if (name.includes('dig') || name.includes('sprawl') || name.includes('pass')) digFocus++;
        if (name.includes('serve')) serveFocus++;
      });
      if (setFocus > 1) { p.shoulders += 1.0; p.arms += 1.0; }
      if (hitFocus > 1) { p.shoulders += 1.0; p.core += 1.0; p.legs += 0.5; }
      if (blockFocus > 1) { p.legs += 1.0; p.shoulders += 0.5; }
      if (digFocus > 1) { p.legs += 1.0; p.core += 1.0; p.back += 0.5; }
      if (serveFocus > 1) { p.shoulders += 1.0; p.core += 0.5; }
    }
...
export function calculateSoreness(dateStr, loggedWorkouts = []) {
  ...
  while (toKey(currentDate) <= dateStr) {
    ...
    dayLogs.forEach(log => {
      ...
      const logSoreness = calculateFatigueImpact(log);
...
export function getRecommendation(dateStr, loggedWorkouts = [], calendarEvents = [], goals = { frequency: 3 }) {
  const todayStr = dateStr;
  const tomorrowStr = addDays(todayStr, 1);
  const yesterdayStr = addDays(todayStr, -1);

  const soreness = calculateSoreness(todayStr, loggedWorkouts);
  ...
  if (hasSportToday) {
    ...
    return {
      ...
      reason: `You have a sport activity today (${sportEvent?.title || 'Game Day'}). To protect your joints and prevent injury, we recommend this prehab and release activation rather than heavy training.`,
      impactEvents: sportEvent ? [sportEvent] : []
    };
  }
  ...
  if (hasSportTomorrow) {
    ...
    return {
      ...
      reason: `You have a game/play scheduled tomorrow (${sportEventTomorrow?.title || 'Game Day'}). We're skipping heavy leg work and jumps today to keep your muscles fresh, explosive, and fatigue-free for tomorrow.`,
      impactEvents: sportEventTomorrow ? [sportEventTomorrow] : []
    };
  }
  ...
  const liftCount7Days = loggedWorkouts.filter(w => {
    ...
    return diffDays >= 0 && diffDays < 7 && (w.type === 'lifting' || w.id?.includes('weightlifting') || w.category === 'weightlifting');
  }).length;

  const weeklyTarget = parseInt(goals.frequency || '3');

  if (liftCount7Days < weeklyTarget) {
