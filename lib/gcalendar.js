...
/**
 * Automatically refresh Google OAuth access token using stored refresh token.
 */
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.access_token) {
        if (typeof window !== 'undefined') {
          localStorage.setItem('nexus_provider_token', data.access_token);
        }
        return data.access_token;
      }
    }
  } catch (err) {
    console.warn('Auto refresh Google token failed:', err);
  }
  return null;
}
 ...
 */
export async function fetchUserCalendars(accessToken, isRetry = false) {
  ...
  try {
    ...
    if (res.status === 401 && !isRetry) {
      const cachedRefresh = typeof window !== 'undefined' ? localStorage.getItem('nexus_provider_refresh_token') : null;
      if (cachedRefresh) {
        const newToken = await refreshGoogleToken(cachedRefresh);
        if (newToken) {
          return fetchUserCalendars(newToken, true);
        }
      }
    }
 ...
 */
export async function fetchCalendarEvents(accessToken, calendarId = 'primary', timeMin, timeMax, isRetry = false) {
  if (!accessToken) return [];
  try {
    ...
    if (res.status === 401 && !isRetry) {
      ...
      if (cachedRefresh) {
        const newToken = await refreshGoogleToken(cachedRefresh);
        if (newToken) {
          return fetchCalendarEvents(newToken, calendarId, timeMin, timeMax, true);
        }
      }
    }
